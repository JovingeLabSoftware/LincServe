...
LINCS.prototype.get = function(keys, view, cb) {
  ...
  if(view) {
    ...
    .then(function(data) {
      var results = [];  // need to flatten array from chunked requests
      data.forEach(function(dd) {
        dd.forEach(function(d) { results.push({id: d.id, metadata: d.value, key: d.key}) });
      });
...
LINCS.prototype._getMultiView = function(keys, view) {
  ...
  while(keys.length>0) {
    var deferred = Q.defer();
    var k = keys.splice(0, LINCS_MAXGET);
    var query = this.view.from(ddoc, view)
      .reduce(false)
      .keys(k);
    this._qq.push({query: query, promise: deferred});
    promises.push(deferred.promise);
    this._goQQ();
  }
  return Q.all(promises);
