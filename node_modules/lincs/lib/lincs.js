 ...
 * @param {string} plate Plate ID to pull controls from
 * @param {string} cell Cell line ID to pull controls from
 * @param {numeric} time Timepoint to pull controls from
 * @param {function} cb Call back to be called upon completion of async call.  Optional--can use promises instead.
 */

LINCS.prototype.instShSamePlateVehicles = function(plate, cell, time, cb) {
	var deferred = Q.defer();
	var query = this.view.from('debug', 'sh_plate_controls').reduce(false)
	            .key([plate, cell, time]);
	this.bucket.query(query, function(err, results) {
	 if(err) {
        deferred.reject(err);
...
/**
 * Get data for set of distil_ids.
 * @param {[string]} ids List of ids. 
 * @param {string} type Data type, such as 'q2norm'
 * @param {function} cb Call back to be called upon completion of async call.  Optional--can use promises instead.
 * @example
 */
LINCS.prototype.getData = function(ids, type, cb) {
	var deferred = Q.defer();
  ids = [].concat(ids);
  var query = this.view.from('lincs', 'data')
    .reduce(false)
    .keys(ids.map(function(x) { return([x, type]) } ));
