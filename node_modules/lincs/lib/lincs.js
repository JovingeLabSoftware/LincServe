...
LINCS.prototype.savePert = function(doc, cb){
  var deferred = Q.defer();
  if(!this._checkParams(doc, ['method', 'dose', 'perturbagen', 'duration', 'gene_ids', 'data', 'type'])){
    ...
    doc.type = "pert";
    var key = doc.method + "_" + doc.cell + "_" +  doc.perturbagen +  
             "_" + doc.dose +  "_" + doc.duration;
    this.bucket.upsert(key, JSON.stringify(doc), function(err, res) 
    {
      if(err) {
        deferred.reject(err);
      } else {
 ...
 * @param {string} doc Document in JSON including type, metadata, gene_ids, 
 *                 expression, gold.  Type should indicate what type of data 
 *                 this is, e.g. "q2norm"
 * @param {function} cb Call back to be called upon completion of async call.  
 *                       Optional--can use promises instead.
 */
LINCS.prototype.saveInstance = function(id, doc, cb){
  var deferred = Q.defer();
  if(!this._checkParams(doc, ['metadata', 'gene_ids', 'data', 'type', 'gold'])){
