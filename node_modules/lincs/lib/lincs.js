...
LINCS.prototype.instSamePlateVehicles = function(key, cb) {
    ...
    var query = _self.view.from('lincs', 'vehicle_by_plate')
        .reduce(false).key( [plate, v.vehicle, v.pert_time]);
...
LINCS.prototype.getExpression = function(key, cb) {
	...
	this.bucket.get(key, function(err, doc) {
		if(err) {
  		...
  		deferred.resolve({gene_ids: doc.value.gene_ids, metadata: doc.value.metadata, norm_exp: doc.value.norm_exp});
		}
	});
	deferred.promise.nodeify(cb);
	return deferred.promise;
...
/**
 * Get a set of doc summaries by NID (numerical ID) or index range.  NOTE: NIDs are NOT unique.  Expect
 ...
 * @param {boolean} NID Are first and last NIDs?  Alternative is document number ("page").
 * @param {function} cb Call back to be called upon completion of async call.  Optional--can use promises instead.
 */
LINCS.prototype.getSummaries = function(first, last, NID, cb) {
  var deferred = Q.defer();
  var query = this.view.from('lincs', 'numerical_index')
    .reduce(false);
  if(NID) {
    query.options.startkey = first;  // query.range does not work because it adds quotes
    query.options.endkey = last;
  } else {
    query = query.skip(first).limit(last-first);    
  }
  this.bucket.query(query, function(err, results) { 
      if(err) {
        deferred.reject(err);
...
LINCS.prototype.getZSVC = function(cell_line, 
  ...
  if(gold) {
    query = this.view.from('lincs', 'ZSVC_L1000_gold')
      .reduce(false).skip(skip || 0).limit(limit || 1000);
  } else {
    query = this.view.from('lincs', 'ZSVC_L1000')
      .reduce(false).skip(skip || 0).limit(limit || 1000);
  }
...
/**
 * Insert a z score document into the document store.  
 * @param {string} id ID of the signature.
 * @param {string} cell_line ID of cell lines. Can be null.
 * @param {string} pert Name of perturbagen. Can be null.
 * @param {numeric} dose Dose (numeric, unitless).  Can be null.
 * @param {numeric} time Duration of exposure (numeric, unitless). Can be null.
 * @param {string} type Type of zscore, e.g. "ZSVC_L1000"
 * @param {string[]} gene_ids Gene identifiers corresponding to scores
 * @param {numeric[]} zscores The scores.
 * @param {boolean} gold Is this a gold signature? Can be null
 * @param {function} cb Call back to be called upon completion of async call.  Optional--can use promises instead.
 */
LINCS.prototype.saveZScores = function(cell_line, 
                                       pert, 
                                       dose, 
                                       time, 
                                       type, 
  ...
  var deferred = Q.defer();
  gold = gold || false;
  if(JSON.parse(gene_ids).length != JSON.parse(zscores).length) {
        deferred.reject(new Error("Gene IDs length must match zscores length"));
  }
  var id = [cell_line, pert, dose, time, type, version].join("_");
  this.bucket.upsert(id, {metadata: {cell_id: cell_line, pert_id: pert, 
                     pert_dose: dose, pert_time: time, type: type, version: version, is_gold: gold},
                     gene_ids: gene_ids,
                     zscores: zscores}, function(err, res) 
  {
    if(err) {
      deferred.reject(err);
    } else {
